...
Alter table Transkacje modify (czas_transkacji defualt systimestamp);
Alter table Kasjerzy add constraint kasjerzy_id_kas_ch check (id_kasjera>=100);
Alter table Transakcje rename column czas_transakcji to czas_transakcji;

--d)
Alter table Kasjerzy modify (placa default 2500);
Alter table Transakcje modify (czas_transkacji defualt systimestamp);
Alter table Kasjerzy add constraint kasjerzy_id_kas_ch check (id_kasjera>=100);
...
--e)
Alter table Transkacje drop constraint id_sprzedawcy_p;
Alter table Transakcje add
...
--g)
Alter table Kasjerzy add (data_urodzenia date constraint data_urodzenia_ch check(data_urodzenia >= to_date
  ('01.01.1960', 'DD.MM.YYYY')), data_zwolnienia date default null);
